...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!speechResult) {
      ...
      // Optimized: Reasonable timeout for thinking, ultra-short speechTimeout for immediate detection
      const dataParam = encodedData ? `&amp;data=${encodeURIComponent(encodedData)}` : '';
      const twiml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Gather input="speech" timeout="5" speechTimeout="0.5" bargein="true" action="${baseUrl}/api/twiml/process-speech?callSid=${callSid}&amp;disputeId=${disputeId}${dataParam}" method="POST">
